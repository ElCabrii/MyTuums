...
test.describe("locale", () => {
	...
	test("legal pages follow the locale — mentions-légales included", async ({
		page,
	}) => {
		...
		await expect(
			page.getByRole("heading", { name: "1. Site publisher" }),
		).toBeVisible();
		...
		await expect(
			page.getByRole("heading", { name: "1. Éditeur du site" }),
		).toBeVisible();
