...
- **The declared MIME type is never trusted.** `packages/api/src/image.ts` sniffs the leading bytes and rejects a mismatch rather than silently correcting it. SVG is absent from the allowlist on purpose — it is a document format that can carry script, and these bytes are served back from our own origin.
- **The client re-encodes through a canvas before uploading** (`apps/web/src/lib/media.ts`), which keeps `sharp` and its native binary out of the server image and guarantees genuine raster bytes. It is the cooperative path, not the security boundary — the server sniffs regardless.
- **`setImageColumn` runs in a transaction with `SELECT ... FOR UPDATE`.** The previous value cannot come from `RETURNING` (that reports the row *after* the update), and two bare statements let concurrent uploads both read the same old key, both delete it, and orphan the loser's object.
- oRPC serialises a `File` as multipart natively, so the procedure takes `z.file()` with no base64 hop.
