...
- **The component test suites were deleted during the Jotai migration and are not yet rewritten** — see `apps/web/TESTS-TO-REWRITE.md`, which enumerates all 59 of them plus the invariants that must survive. They module-mocked `@/lib/auth-client`, which the session atoms made inert. Rewrites should seed a `createStore()` and wrap in `<Provider store>` instead of mocking that module. Current coverage is the pure layers: cache helpers, atoms, validation, and `lib/format` / `lib/user`.
...
- **`useToggleLike` and `useToggleFollow` are deliberately still `useMutation` hooks.** `atomWithMutation` does forward per-call `MutateOptions`, but query-core only fires those callbacks when `hasListeners()` is true (`mutationObserver.ts:164`), and `observer.subscribe` runs only in the result atom's `onMount`. A write-only action atom read with `useSetAtom` never mounts it, so `mutate(vars, { onError })` — which is where both rollbacks live — is silently dropped and a failed like sticks forever. Confirmed empirically: mounted via `store.sub`, `onError` fires; unmounted write-only, it does not. Migrating them requires the component to mount the mutation atom via `useAtomValue`, accepting the re-renders `follow-button.tsx` deliberately designed away, *or* rewriting rollback around `onMutate` context.
