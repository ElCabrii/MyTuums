/**
 * Grants the Founder badge — the one manually-granted badge, on the three
 * founder accounts (issue #308).
 *
 * There is deliberately no in-app surface for this: the Founder badge is
 * granted out of band, once per account, in the same spirit as the first
 ...
 * which owns the actual work and the three-account guard, so the local flow
 * and the production entry point cannot drift apart.
 */
