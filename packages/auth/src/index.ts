...
import type { AuthRateLimitStorage } from "./rate-limit-storage.js";
...
import type { OAuthCredentials } from "./env.js";
...
import { createSocialProviders } from "./social.js";
...
export interface AuthOptions {
  ...
  providers?: { google?: OAuthCredentials; discord?: OAuthCredentials; twitch?: OAuthCredentials };
}

/** Bind auth to this environment's database, origin and mail transport. */
export function createAuth({ db, ...options }: AuthOptions) {
  return configureAuth({
    ...options,
    database: drizzleAdapter(db, { provider: "sqlite" }),
    onUserCreated: (id) => stampJoinBadges(db, id),
  });
}

/** Shared with schema generation so its plugins and additional fields cannot drift. */
export function configureAuth({
  ...
  onUserCreated: (id: string) => Promise<void>;
}) {
  const passkeyRpId = new URL(webOrigin).hostname;
  const { socialProviders, trustedProviders } = createSocialProviders(providers);
  return betterAuth({
    baseURL: webOrigin,
    secret,
    ...
    // Dependency messages and arguments can contain SQL parameters, provider
    // responses and email capabilities. Keep severity without forwarding them.
    logger: {
      level: "warn",
      log(level) {
        const event = { event: "auth_diagnostic", level };
    ...
    onAPIError: {
      onError(error) {
        // Better Auth otherwise also uses a global logger in this path,
        // bypassing the configured logger for some API errors.
        if (!isAPIError(error)) {
          console.error({ event: "auth_request_failed" });
          // Better Call otherwise logs the original failure after this callback.
          // Its APIError path preserves a 500 without printing SQL or providers.
          throw new APIError("INTERNAL_SERVER_ERROR", {
            code: "INTERNAL_SERVER_ERROR",
            message: "Internal Server Error",
          });
        }
        if (error.statusCode >= 500) console.error({ event: "auth_request_failed" });
      },
    },
    ...
    emailAndPassword: {
      ...
      sendResetPassword: async ({ user, url }, request) => {
        await sendEmail({
          to: user.email,
          ...(await passwordResetEmail(webOrigin, url, localeFromRequest(request?.headers))),
        });
      },
    },
    ...
    emailVerification: {
      ...
      sendVerificationEmail: async ({ user, url }, request) => {
        await sendEmail({
          to: user.email,
          ...(await verificationEmail(webOrigin, url, localeFromRequest(request?.headers))),
        });
      },
    },
    ...
    plugins: [
      ...
      twoFactor({
        ...
        // second option through the configured native email transport. Delivery
        // failures are sanitized; codes must never be printed in diagnostics.
        otpOptions: {
          sendOTP: async ({ user, otp }, ctx) => {
            await sendEmail({
              ...
              ...(await otpEmail(
                webOrigin,
                otp,
                localeFromRequest(ctx?.request?.headers ?? ctx?.headers),
              )),
            });
          },
        },
