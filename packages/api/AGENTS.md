...
- **Upload order: write the row, then delete the object.** The display and
  original variants share one uuid with an `.orig` infix, and
  `objectKeyFromMediaPath` returns `null` for provider URLs.
