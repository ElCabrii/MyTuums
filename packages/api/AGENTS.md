...
- `src/moderation.ts` — the moderation router (issue #38): reports, blocks, the staff actions and the audit log. The triage procedures (queue, case, resolve) live in `moderation-queue.ts` and the two appeal procedures in `moderation-appeals.ts`; this file assembles all three into `moderationRouter`.
- `src/moderation-queue.ts` — the triage procedures: the merged queue, the case view, and resolving a case. The queue merges report groups and open appeals in JS with a single keyset cursor; each side carries a correlated not-exists exclusion so a dual report+appeal case is never re-emitted across pages. The one paginated list that does NOT go through `keysetPage` — the JS merge does not fit the single-query skeleton.
- `src/moderation-appeals.ts` — the two appeal procedures: `appealOpen` (the app's ONE public surface, HMAC-capability gated) and `appealReview`.
- `src/moderation-inputs.ts` — `noteInput` and `queueInput`, the two input schemas the moderation router files share. A leaf module on purpose: the router files must never import each other (a cycle would fail at module evaluation).
...
- `src/pagination.ts` — `keysetPage`, the keyset page skeleton every paginated list is built from: the row-value cursor filter (with the `sql.param` serialization rule), the hasMore decision, and the last-returned anchor. The caller supplies the query — selection, joins, filters, ORDER BY on the same columns as the cursor comparison, and the +1 lookahead. The selection is passed to the helper too: `createdAtField` is type-tied to the `createdAt` column (the field must be the selection key whose value IS that column), so a cursor can never encode a different timestamp than the SQL compares.
