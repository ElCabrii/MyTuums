...
describe("createRateLimiter", () => {
  it("allows exactly `limit` requests per window and denies the next", async () => {
    ...
    expect((await limiter.consume("k", p)).allowed).toBe(true);
    expect((await limiter.consume("k", p)).allowed).toBe(true);
    expect((await limiter.consume("k", p)).allowed).toBe(true);
    expect((await limiter.consume("k", p)).allowed).toBe(false);
  });

  it("remaining counts down and clamps at 0 instead of going negative", async () => {
    const limiter = createRateLimiter({ now: () => 0 });
    const p = policy({ limit: 2 });

    expect((await limiter.consume("k", p)).remaining).toBe(1);
    expect((await limiter.consume("k", p)).remaining).toBe(0);
  ...
  it("retryAfterSeconds is 0 while allowed and floors at 1 right at the reset boundary", async () => {
    let clock = 0;
    const limiter = createRateLimiter({ now: () => clock });
    const p = policy({ limit: 1 });

    const first = await limiter.consume("k", p); // resetAt = 1000
    expect(first.allowed).toBe(true);
    expect(first.retryAfterSeconds).toBe(0);

    clock = 999; // 1ms before rollover — the raw (resetAt-at)/1000 is tiny
    const second = await limiter.consume("k", p);
  ...
  it("gives distinct keys independent buckets", async () => {
    ...
    expect((await limiter.consume("a", p)).allowed).toBe(true);
    expect((await limiter.consume("a", p)).allowed).toBe(false);
    expect((await limiter.consume("b", p)).allowed).toBe(true);
  });

  it("allows again once the window rolls over", async () => {
    let clock = 0;
    const limiter = createRateLimiter({ now: () => clock });
    const p = policy({ limit: 1 });
  ...
  it("permits a 2x burst across a window boundary — documented trade-off, not a bug", async () => {
  ...
  describe("sweep", () => {
    it("is lazy: an expired key survives until a full map needs to insert a new one", async () => {
      let clock = 0;
      const limiter = createRateLimiter({ now: () => clock, maxKeys: 2 });
      const p = policy({ limit: 5 });

      await limiter.consume("a", p); // size 1, resetAt 1000
      await limiter.consume("b", p); // size check happens BEFORE this insert (size
      ...
      await limiter.consume("c", p); // new key, map is full (size 2 >= maxKeys 2)
  ...
  describe("maxKeys", () => {
    it("allows a brand-new key through when the map is at capacity with live windows", async () => {
      ...
      const third = await limiter.consume("c", p);
      expect(third.allowed).toBe(true);
      expect(third.remaining).toBe(2);
      expect(limiter.size).toBe(3);
    });

    it("logs the capacity warning once per episode, not once per request", async () => {
      const clock = 0;
      const limiter = createRateLimiter({ now: () => clock, maxKeys: 2 });
      const p = policy({ limit: 10 });
      const warn = vi.spyOn(console, "warn").mockImplementation(() => {});

      await limiter.consume("a", p);
      await limiter.consume("b", p);
      expect(warn).not.toHaveBeenCalled();

      // Map is at capacity now; every further brand-new key logs, but only
      // the first one of this episode.
      await limiter.consume("c", p);
      await limiter.consume("d", p);
      await limiter.consume("e", p);
      expect(warn).toHaveBeenCalledTimes(1);
    });

    it("does not re-log when the map merely wobbles below maxKeys and back within the cooldown", async () => {
      ...
      await limiter.consume("a", short); // resetAt 10
      await limiter.consume("b", short); // resetAt 10
      expect(limiter.size).toBe(2);

      // Map is at capacity (2 >= maxKeys 2): first warning of the episode.
      await limiter.consume("c", long); // resetAt 1000
      expect(limiter.size).toBe(3);
      expect(warn).toHaveBeenCalledTimes(1);
      ...
      clock = 15;
      await limiter.consume("d", long); // resetAt 1015
      ...
      clock = 16;
      await limiter.consume("e", long);
      expect(limiter.size).toBe(3);
      expect(warn).toHaveBeenCalledTimes(1);
    });

    it("logs again once the cooldown elapses, even if the map never dips at all", async () => {
      ...
      await limiter.consume("a", p);
      await limiter.consume("b", p);
      await limiter.consume("c", p); // at capacity (2 >= 2): first warning
      expect(limiter.size).toBe(3);
      expect(warn).toHaveBeenCalledTimes(1);
    ...
    it("recycles a returning caller's expired slot even when the map holds a live window", async () => {
      let clock = 0;
      const limiter = createRateLimiter({ now: () => clock, maxKeys: 2 });
      const p = policy({ limit: 3 });

      await limiter.consume("a", p); // resetAt 1000
      clock = 500;
      await limiter.consume("b", p); // resetAt 1500
    ...
    it("lets expired windows free capacity for a brand-new key", async () => {
      let clock = 0;
      const limiter = createRateLimiter({ now: () => clock, maxKeys: 2 });
      const p = policy({ limit: 3 });
      ...
      expect((await limiter.consume("c", p)).allowed).toBe(true);
      expect(limiter.size).toBe(1);
      expect((await limiter.consume("d", p)).allowed).toBe(true);
      expect(limiter.size).toBe(2);
    });
  });
  ...
  it("clear() empties the map", async () => {
    ...
    await limiter.consume("b", policy());
    expect(limiter.size).toBe(2);
...
describe("RATE_LIMITS", () => {
  it("defines fifteen tiers with distinct names", () => {
