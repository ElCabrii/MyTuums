...
moderation notice unread again. Production runs it automatically: the
`mytuums-prune-notifications` Railway cron service (Nixpacks build of this
repo, `DATABASE_URL` referencing the production Postgres) fires the command
