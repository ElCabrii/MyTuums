 ...
 * carries the pre-login destination (see `validateSearch` in the route file).
 *
 * `login.test.tsx` mounts it from this file to drive the two banned-account
 * navigate sites it owns — the house pattern for tested pages (see
 * `home-page.tsx`).
 */
export function LoginPage() {
  // Both search params arrive from *outside* (see `validateSearch` in the
  // route file) — read once, destructured.
  const { redirect: redirectFromSearch, error: oauthError } = routeApi.useSearch();
