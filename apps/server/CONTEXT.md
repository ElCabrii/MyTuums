...
- **The `/rpc` admission gates run before oRPC buffers.** oRPC buffers a
  ...
  check — so three checks hold ahead of it, all on the declared
  Content-Length:
  1. the hard ceiling, `RPC_MAX_BODY_BYTES`, which 413s;
  2. the pre-auth gate at `RPC_SMALL_BODY_BYTES`. Every procedure takes either
     ...
     so a body above that line on a non-appeal path must present a valid
     session or get 401 — an anonymous upload-sized body is refused instead of
     buffered and then rejected. The one public procedure,
     `moderation.appealOpen`, is exempt from the session demand and gets that
     bound as its own 413 limit instead. The gate reads the **canonical**
     path, so an encoded spelling is recognised and an uncanonicalisable one
     falls to the session demand;
  3. `MAX_RPC_IN_FLIGHT` concurrent dispatches, over which `/rpc` answers 503.
     Chunked bodies carry no Content-Length and so skip (1) and (2): they are
     bounded at the ceiling by oRPC's `BodyLimitPlugin` (wired in `src/index.ts`)
     and in number by (3), which is what keeps concurrent buffering finite.
