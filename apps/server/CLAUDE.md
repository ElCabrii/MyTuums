...
- `src/request-handler.ts` — the routing decision tree, unit-tested with stand-ins. Exact-match `/health`, `/api/auth`, `/rpc` (body cap before oRPC buffers — Content-Length at the router, chunked bounded by oRPC's `BodyLimitPlugin`), `/media` (GET/HEAD only, session required before the key is even parsed → presigned 302, or 401), then the page gate (every extension-less GET/HEAD not on `SIGNED_OUT_PATHS` requires a live session, checked via the same injected `hasValidSession`), static files last, 404, and the catch-all 500 / socket-destroy safety net.
