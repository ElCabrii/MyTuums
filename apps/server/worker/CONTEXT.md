...
Eligible immutable image bytes and generated variants carry a Workers Cache API
layer (issue #405). The per-request D1 authorization runs before the cache
lookup and again before cached bytes are delivered, so the cache can never
substitute for the database. The cache key is a synthetic canonical GET under
the deployment's own origin built from the object key alone — cookies, session
headers, query strings and viewer identity cannot vary it. Profile originals
(`.orig`) are excluded. The stored copy is an internal response with
`public, max-age` (six hours) because the Cache API refuses uncachable
policies; the browser-facing response is built separately and stays
`private, no-store`. Population runs through `ExecutionContext.waitUntil`
(threaded from `index.ts` through the request handler's optional second
argument), so a miss is not delayed by the write. Any cache failure —
including runtimes without a functional Cache API, such as Workers behind
Cloudflare Access in preview — falls back to the uncached R2 path. Only
aggregate hit/miss/error events are observed, never keys or viewer ids.
